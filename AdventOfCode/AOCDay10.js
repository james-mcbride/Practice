...
for (var i=0; i<finalArray.length; i++){
    var pathsCounter=0;
    //will create a sub-array of indexes to check. If two out of three of these index
    var checkArray=finalArray.slice(i+1, i+100)

    for (var k =0; k<branchArray.length; k++){
        var changeCounter=0;
        for (var j=0; j<checkArray.length; j++) {
            if (checkArray[j] - branchArray[k][0] <= 3 && checkArray[j] - branchArray[k][0] >0) {
                newBranchArray.push([checkArray[j], branchArray[k][1]])
                pathsCounter+=branchArray[k][1];
                changeCounter++
            }
        }
        if (changeCounter===0){
            newBranchArray.push(branchArray[k]);
            pathsCounter+=branchArray[k][1]
        }

    }
var currentBranchTotal=branchArray.reduce(function(total, x){
    return total+x[1]
},0)
// if (pathsCounter>0) {
//     pathsTotal *=pathsCounter/currentBranchTotal
// }
branchArray=[];
var uniqueBranches=newBranchArray.map(function(x){
    return x[0]
})
for (var j=0; j<uniqueBranches.length; j++){
    if (uniqueBranches.indexOf(uniqueBranches[j])===j){
        branchArray.push([uniqueBranches[j], 0])
    }
}
for (var j=0; j<branchArray.length; j++) {
    for (var k = 0; k < newBranchArray.length; k++) {
        if (newBranchArray[k][0] === branchArray[j][0]) {
            branchArray[j][1]+=newBranchArray[k][1]
        }
    }
}
    newBranchArray=[];
